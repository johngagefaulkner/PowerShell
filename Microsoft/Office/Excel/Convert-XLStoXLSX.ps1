...
function Convert-XlsToXlsx
{
  ...
  process
  {
    foreach($_ in $Path)
    {
      ...
      if ($exists)
      {
        Write-Host "[WARN] File exists and -Force was not specified, skipping: $_" -ForegroundColor Yellow
        Write-Host "[WARN] File exists. Use -Force to overwrite. $_" -ForegroundColor Yellow
        continue
      }
      ...
      Write-Host "[Success] Converted: '$_'" -ForegroundColor Green
    }
  }
